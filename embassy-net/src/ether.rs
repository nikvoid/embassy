...
use smoltcp::socket::ether;

pub use smoltcp::wire::EthernetProtocol;
pub use smoltcp::socket::ether::PacketMetadata;
...
/// An Ethernet Raw socket.
pub struct EtherSocket<'a> {
    stack: Stack<'a>,
    handle: SocketHandle,
}

impl<'a> EtherSocket<'a> {
    /// Create a new Raw socket using the provided stack and buffers.
    pub fn new(
        stack: Stack<'a>,
        filter: fn(EthernetProtocol) -> bool,
        ...
        let handle = stack.with_mut(|i| {
            ...
            i.sockets.add(ether::Socket::new(
                filter,
                ether::PacketBuffer::new(rx_meta, rx_buffer),
                ether::PacketBuffer::new(tx_meta, tx_buffer),
            ))
        });
    ...
    fn with_mut<R>(&self, f: impl FnOnce(&mut ether::Socket, &mut Interface) -> R) -> R {
        self.stack.with_mut(|i| {
            let socket = i.sockets.get_mut::<ether::Socket>(self.handle);
    ...
    pub fn poll_recv(&self, buf: &mut [u8], cx: &mut Context<'_>) -> Poll<Result<usize, RecvError>> {
        self.with_mut(|s, _| match s.recv_slice(buf) {
            ...
            // No data ready
            Err(ether::RecvError::Truncated) => Poll::Ready(Err(RecvError::Truncated)),
            Err(ether::RecvError::Exhausted) => {
                s.register_recv_waker(cx.waker());
                Poll::Pending
            }
        })
    ...
    pub fn poll_send(&self, buf: &[u8], cx: &mut Context<'_>) -> Poll<()> {
        self.with_mut(|s, _| match s.send_slice(buf) {
            ...
            Ok(()) => Poll::Ready(()),
            Err(ether::SendError::BufferFull) => {
                s.register_send_waker(cx.waker());
                Poll::Pending
            }
        })
...
impl Drop for EtherSocket<'_> {
    fn drop(&mut self) {
        self.stack.with_mut(|i| i.sockets.remove(self.handle));
    }
}

fn _assert_covariant<'a, 'b: 'a>(x: EtherSocket<'b>) -> EtherSocket<'a> {
    x
}
